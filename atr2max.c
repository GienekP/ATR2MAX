...
/*--------------------------------------------------------------------*/
#include "starter128m.h"
#include "starter256m.h"
/*--------------------------------------------------------------------*/
...
{
	U8 header[16]={0x43, 0x41, 0x52, 0x54, 0x00, 0x00, 0x00, 0x29,
...
void atr2max(const char *atrfn, const char *carfn, U8 m, U8 f, U8 d)
{
	...
	if (atrsize)
	{
		...
		if (((sector==SEC128) || (sector==SEC256)))
		{
			...
			{
				printf("Sector: 128\n");
				sum=buildCar128(starter128m_bin,starter128m_bin_len,atrdata,atrsize,cardata,carsize);
			}
			else
			{
				printf("Sector: 256\n");
